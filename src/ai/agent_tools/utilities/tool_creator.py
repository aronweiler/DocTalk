...
def create_tool(tool_json = None, memory = None, override_llm = None) -> Tool:
    ...
    if "arguments" in tool_json:
        tool_instance.configure(memory=memory, override_llm=override_llm, json_args=tool_json["arguments"])
    else:
        tool_instance.configure(memory=memory, override_llm=override_llm)
