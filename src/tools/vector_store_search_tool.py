...
class VectorStoreSearchTool:
    ...
    def run(self, search_terms) -> str:
        ...
        for db in self.databases:
            #  vectordbkwargs = {"search_distance": .5, "k": len(db.get()["documents"]), "search_type": "mmr"}
            #  retriever = db.as_retriever(search_kwargs=vectordbkwargs)
...
searcher = VectorStoreSearchTool([ "pr", "pr_reports"], False, False)    
result = searcher.run("Rene Smith 2022 2023")
print(result)