...
class LocalVectorStoreTool:
    ...
    def __init__(self, database_name, top_k = 4, search_type = "mmr", search_distance = .5, verbose = False, max_tokens = shared.MAX_LOCAL_CONTEXT_SIZE, return_source_documents = False):
            ...
            db = get_database(get_embedding(True), database_name)    
            ...
            print(f"LocalVectorStoreTool initialized with database_name={database_name}, top_k={top_k}, search_type={search_type}, search_distance={search_distance}, verbose={verbose}, max_tokens={max_tokens}")

    def run(self, query:str) -> str:
        print("LocalVectorStoreTool got: ", query)
